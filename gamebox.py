...
def load_sprite_sheet(url_or_filename, rows, columns):
    """Loads a sprite sheet. Assumes the sheet has rows-by-columns evenly-spaced images and returns a list of those
    images. """
...
def from_text(x, y, text, font_size, color, bold=False, italic=False):
...
class SpriteBox(object):
    """Intended to represent a sprite (i.e., an image that can be drawn as part of a larger view) and the box that
    contains it. Has various collision and movement methods built in. """
    ...
    def bottom_touches(self, other, padding=0, padding2=None):
        """b1.bottom_touches(b2) returns True if both b1.touches(b2) and b1's bottom edge is the one causing the
        overlap. """
        if padding2 is None: padding2 = padding
    ...
    def right_touches(self, other, padding=0, padding2=None):
        """b1.right_touches(b2) returns True if both b1.touches(b2) and b1's right edge is the one causing the
        overlap. """
        if padding2 is None: padding2 = padding
        return self.overlap(other, padding + 1, padding2 + 1)[0] < 0
    ...
    def move_to_stop_overlapping(self, other, padding=0, padding2=None):
        """b1.move_to_stop_overlapping(b2) makes the minimal change to b1's position necessary so that they no longer
        overlap """
